...
# Log dos valores usados no merge
log_message("Valores usados no merge (SUAP):")
log_message(suap_df[['CPF_NORMALIZADO', 'NOME_NORMALIZADO']].head().to_string())
log_message("Valores usados no merge (PNP):")
log_message(pnp_df[['CPF_NORMALIZADO', 'NOME_NORMALIZADO']].head().to_string())

# Merge com validação de período
merged = pd.merge(
    suap_df,
    pnp_df,
...
# Log das colunas do DataFrame após o merge
log_message(f"Colunas após o merge: {merged.columns.tolist()}")
...
merged['VALID_PERIOD'] = merged.apply(
    lambda row: (
        ...
        log_message(f"Validação de período: {(int(row['ANO DO CICLO']) == int(row['ANO DE INGRESSO'])) and (row['DATA_DE_MATRICULA'].year == int(row['ANO DE INGRESSO']))}") or
        pd.notna(row['ANO DO CICLO']) and  # Verifica se não é NaN
        pd.notna(row['ANO DE INGRESSO']) and  # Verifica se não é NaN
        (int(row['ANO DO CICLO']) == int(row['ANO DE INGRESSO'])) and  # Compara anos
        (row['DATA_DE_MATRICULA'].year == int(row['ANO DE INGRESSO']))  # Compara ano de matrícula
    ),
    axis=1
)

# Geração dos arquivos de saída
correlated = merged[(merged['_merge'] == 'both') & (merged['CURSO_VALIDO']) & (merged['VALID_PERIOD'])]
